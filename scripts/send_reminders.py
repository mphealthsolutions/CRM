...
TYPE_META = {
    "follow_up": {"icon": "📅", "label": "Follow-Up Due",      "color": "#1d4ed8"},
    "check_in":  {"icon": "🔔", "label": "Check-In Overdue",   "color": "#b45309"},
    "renewal":   {"icon": "📄", "label": "Contract Renewal",   "color": "#15803d"},
}
...
def build_email(reminders, ai_summary=None):
    ...
    for r in reminders:
        ...
        <tr>
          <td style="padding:14px 16px;border-bottom:1px solid #f0f0f0;">
            ...
            <div style="font-size:15px;font-weight:600;color:#111;">{c.get('name','')}</div>
            <div style="font-size:13px;color:#666;">{c.get('company','')} · {c.get('title','')}</div>
            <div style="font-size:12px;color:#999;margin-top:2px;">{r['message']}</div>
          </td>
          <td style="padding:14px 16px;border-bottom:1px solid #f0f0f0;
                     vertical-align:top;white-space:nowrap;">
            <a href="mailto:{c.get('email','')}"
               style="display:inline-block;padding:6px 14px;background:#0f172a;
    ...
    if ai_summary:
        ai_block = f"""
        <div style="margin:24px 0;padding:18px 20px;background:#f8fafc;
...
<html><body style="margin:0;padding:0;background:#f1f5f9;font-family:'Georgia',serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;
    ...
    <div style="background:#0f172a;padding:28px 32px;">
      <div style="font-size:11px;letter-spacing:.12em;color:#94a3b8;
                  text-transform:uppercase;margin-bottom:4px;">MP Health Solutions</div>
      <div style="font-size:22px;font-weight:700;color:#fff;">
        CRM Daily Digest
      </div>
      <div style="font-size:13px;color:#64748b;margin-top:4px;">{today}</div>
    </div>

    <!-- Summary bar -->
    <div style="background:#f8fafc;padding:14px 32px;border-bottom:1px solid #e2e8f0;">
      <span style="font-size:13px;color:#475569;">
        {f'<strong>{count} reminder{"s" if count != 1 else ""}</strong> require your attention today.'
         if count else '<strong>All clear</strong> — no reminders due today.'}
      </span>
    </div>
